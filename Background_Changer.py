...
# Connects the GUI to the database when a button is pressed, the host, user, password, db name and port number are also in the def below.
def Connect_to_Database():
    try:
        ...
        sql_statement = "SELECT netName FROM etop"
        my_database.execute(sql_statement)
        output = connection.fetch_eof_columns()
        print(output)
...
network3 = [[sg.Text('Network gifs', size=(10, -5))],
            ...
            [sg.Multiline(f'{Data}', key='acss', size=(60, 10))]
            ]
...
tablayout = [[sg.TabGroup([[sg.Tab('Presets', layout2, key='presetTab'),
                            ...
                            sg.Tab('Database', network3, key='dta')
                            ]], enable_events=True, key="tab")]]

# Main GUI layout (Tabbed)
...
#  I am met with "2005 (HY000): Unknown MySQL server host '10.277.160.98:3306' (11001)" each time the button is pressed.
    elif event == 'acss':
        Connect_to_Database()
        print()
